...
;(jQuery.ov && jQuery.ov.json) || (function( $) {
	...
	function domSymbol( selector) {
		...
		return selector.length>0 ? selector.map( function() {
	...
	jQuery.ov.json = {
			stringify : function( obj, options) {
				...
				// -> make it
			if( !$.isArray( this)) {
				return jQuery.ov.json.stringify.call( [], obj, options);
			}
			...
			if( obj===window) {
				...
				var i, n, v, json = [], arr = $.isArray( obj);
				...
				for( i=0; i<properties.length; i++) {
					...
					if (v!==null) {
						if( t!='object' || $.inArray( v, this)==-1) {
							...
							v = jQuery.ov.json.stringify.call(
								this,
								v,
								_options
							);
